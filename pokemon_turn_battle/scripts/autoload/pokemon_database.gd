...
func _build_database() -> void:
	...
	# --- Hisui (Legends: Arceus) — nomes/tipos reais das formas evoluídas
	# de Hisui, mas com status recalibrados para nível de inicial (ver nota
	# no README: os status reais desses três são de Pokémon já evoluído, e
	# usá-los sem ajuste dava vantagem injusta sobre qualquer outro inicial) ---
	var hisui: Array[PokemonSpecies] = [
		_make_species(id, "Decidueye de Hisui", "Hisui", "Grama", "Lutador", 68, 65, 55, 68, Color(0.35, 0.6, 0.35), "Folha Navalha", 45),
		_make_species(id + 1, "Typhlosion de Hisui", "Hisui", "Fogo", "Fantasma", 60, 60, 50, 70, Color(0.55, 0.35, 0.75), "Chama Fantasma", 45),
		_make_species(id + 2, "Samurott de Hisui", "Hisui", "Água", "Sombrio", 65, 68, 58, 50, Color(0.25, 0.4, 0.55), "Corte Noturno", 45),
	]
